...
class Util{
	...
	static pointRayCrossesLine(point: THREE.Vector3, line: THREE.Line3, prevDirection: boolean, thisDirection: boolean) {
	...
	static pointRayCrossesSegments(point: THREE.Vector3, segments: THREE.Line3[]) {
		let crossings = 0;
		...
		for (let s = 0, l = segments.length; s < l; s++) {
		  ...
		  if (Util.pointRayCrossesLine(point, line, prevDirection, thisDirection)) {
			crossings++;
		  }
