...
class PLYLoader extends Loader {
	...
	parse( data ) {
		...
		function handleElement( buffer, currentElementCount, elementName, element, cacheEntry ) {
			
			if ( elementName === 'vertex' ) {
	
				// buffer.vertices.push( element[ cacheEntry.attrX ], element[ cacheEntry.attrY ], element[ cacheEntry.attrZ ] );
                buffer.vertices[currentElementCount * 3 + 0] = element[ cacheEntry.attrX ];
                buffer.vertices[currentElementCount * 3 + 1] = element[ cacheEntry.attrY ];
                buffer.vertices[currentElementCount * 3 + 2] = element[ cacheEntry.attrZ ];
				...
				if ( cacheEntry.attrNX !== null && cacheEntry.attrNY !== null && cacheEntry.attrNZ !== null ) {
                    ...
                    buffer.normals[currentElementCount * 3 + 0] = element[ cacheEntry.attrNX ];
                    buffer.normals[currentElementCount * 3 + 1] = element[ cacheEntry.attrNY ];
                    buffer.normals[currentElementCount * 3 + 2] = element[ cacheEntry.attrNZ ];
				...
				if ( vertex_indices.length === 3 ) {
					...
					console.log(vertex_indices,"vertex_indices")
		...
		function parseBinary( data, header ) 
		{
			...
			for ( let currentElement = 0; currentElement < header.elements.length; currentElement ++ ) {
                ...
                buffer.vertices.length = elementDesc.count * 3;
                buffer.normals.length = elementDesc.count* 3;
				...
				for ( let currentElementCount = 0; currentElementCount < elementDesc.count; currentElementCount ++ ) {
					...
					handleElement( buffer, currentElementCount, elementDesc.name, element, attributeMap );
			...
			try{
				buffer.vertices = buffer.vertices.filter(item => (
					(typeof item === 'number' && isFinite(item)) || item === 0
				  ));
			}catch(e){
				console.log(e)
			}
		...
		if ( data instanceof ArrayBuffer ) {
			...
			if ( header.format === 'ascii' ) {
				...
				// console.log(data, header,'asdfasfsd')
